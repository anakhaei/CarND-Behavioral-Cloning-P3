...
### Model Architecture
I have started with an Architecture similar to NVIDIA paper and I fine tuned it. Here are the steps that I took to improve the architecture and the model:
* Intially I trained my network with only center camera of Udacity data set. I also removed some of the convolutional layesr to keep more pixels before my fully connected layer. The model was not able to handle the turn before the bridge.
* Then I intoduced the images from left and right cameras with the correction of 0.1. Probbaly the correction was not enough to prevent the car going out of the lane.
* I introduced two changes at the same time so it is now difficault the judje the impact of each of them. I increased the correction factor to 0.3 and also augmented the data set by flipping them. It was a major improvement, but still not good enough to handle all the turns.
* At this point, I added my own data set by driving 2 laps and also several recovery cases from right and left. At this time, I got a good result with smooth manouvers. I tested it with the velocity of 10 and 20 it performed quite well. No success at the velocity of 30!  
